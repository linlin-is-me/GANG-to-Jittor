...
def rotation_between_z(vec):
    ...
    R = jt.zeros(vec.shape[:-1] + (3, 3,), dtype=jt.float32)
    ...
    # jt.eye not available in Jittor — construct identity manually
    identity_3x3 = jt.zeros(3, 3, dtype=jt.float32)
    identity_3x3[0, 0] = identity_3x3[1, 1] = identity_3x3[2, 2] = 1.0
    R = jt.where((vec[..., 2] + 1 > 0)[..., None, None], R,
                    -identity_3x3.expand_as(R))
    return R
...
def eval_sh_coef(deg, dirs):
    ...
    assert 4 >= deg >= 0
    coeff = (deg + 1) ** 2
    results = jt.zeros(dirs.shape[:-1] + (coeff,))
    results[..., 0] = C0
    if deg > 0:
