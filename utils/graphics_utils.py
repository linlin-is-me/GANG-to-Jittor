...
def geom_transform_points(points, transf_matrix):
    ...
    points_hom = jt.concat([points, ones], dim=1)
...
def getWorld2View2(R, t, translate=np.array([.0, .0, .0]), scale=1.0):
    ...
    cam_center = C2W[:3, 3].copy()  # .copy() avoids non-contiguous segfault with jt.Var
...
def rgb_to_srgb(img, clip=True):
    ...
    if isinstance(img, np.ndarray):
        ...
        img = jt.where(img > 0.0031308, jt.pow(jt.maximum(img, jt.array(0.0031308)), 1.0 / 2.4) * 1.055 - 0.055, 12.92 * img)
        if clip:
            img = jt.clamp(img, 0.0, 1.0)
        return img
    else:
        raise TypeError("Unsupported input type. Supported types are numpy.ndarray and jt.Var.")
...
def fibonacci_sphere_sampling(normals, sample_num, random_rotate=True):
    ...
    idx = jt.arange(sample_num, dtype=jt.float)[None]
    z = (1 - 2 * idx / (2 * sample_num - 1)).clamp_min(np.sin(10/180*np.pi))
    rad = jt.sqrt(1 - z ** 2)
    theta = delta * idx
    if random_rotate:
        theta = jt.rand(*pre_shape, 1) * 2 * np.pi + theta
    y = jt.cos(theta) * rad
    x = jt.sin(theta) * rad
    z_samples = jt.stack([x, y, z.expand_as(y)], dim=-2)

    # rotate to normal
    # z_vector = jt.zeros_like(normals)
    # z_vector[..., 2] = 1  # [H, W, 3]
    ...
    incident_dirs = jt.normalize(incident_dirs, dim=-2).transpose(-1, -2)
    incident_areas = jt.ones_like(incident_dirs)[..., 0:1] * 2 * np.pi
    if len(pre_shape) > 1:
...
def ndc_2_cam(ndc_xyz, intrinsic, W, H):
    ...
    cam_xyz = jt.concat([cam_xy, cam_z], dim=-1)
    cam_xyz = cam_xyz @ jt.linalg.inv(intrinsic[0, ...].t())
    return cam_xyz
...
def srgb_to_rgb(img):
    ...
    if isinstance(img, np.ndarray):
        ...
        assert len(img.shape) == 3, img.shape
        assert img.shape[0] == 3, img.shape
        img = jt.where(img <= 0.04045, img / 12.92, jt.pow((jt.maximum(img, jt.array(0.04045)) + 0.055) / 1.055, 2.4))
        return img
    else:
        raise TypeError("Unsupported input type. Supported types are numpy.ndarray and jt.Var.")
...
def depth2point_world(depth_image, intrinsic_matrix, extrinsic_matrix):
    ...
    xyz_world = jt.concat([xyz_cam, jt.ones_like(xyz_cam[...,0:1])], axis=-1) @ jt.linalg.inv(extrinsic_matrix).transpose(0,1)
...
#     sorted_idx = jt.argsort(scales, descending=False, dim=-1)
#     R = build_rotation(rotations)
#     R_sorted = jt.gather(R, dim=2, index=sorted_idx[:,None,:].repeat(1, 3, 1)).squeeze()
...
def flip_align_view(normal, viewdir):
    ...
    normal_flipped = normal * (non_flip.float() * 2 - 1)  # True→1, False→-1 (avoids jt.where CUDA-only)
