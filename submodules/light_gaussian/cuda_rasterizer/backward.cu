  ...
  glm::vec4* dL_drot)
{
  ...
  // === Inlined computeCov3D (Jittor 1.3.11 tracer can't trace __device__ calls) ===
  {
    const glm::vec3 cov_scale = scales[idx];
    const float cov_mod = scale_modifier;
    const glm::vec4 cov_rot = rotations[idx];
    glm::vec4 q = cov_rot;
    ...
    glm::mat3 R = glm::mat3(
      ...
      2.f * (x*z - r*y), 2.f * (y*z + r*x), 1.f - 2.f * (x*x + y*y));
    glm::mat3 S = glm::mat3(1.0f);
    glm::vec3 s = cov_mod * cov_scale;
    S[0][0] = s.x; S[1][1] = s.y; S[2][2] = s.z;
    glm::mat3 M = S * R;
    const float* dcd = dL_dcov3D + 6 * idx;
    glm::mat3 dL_dSigma = glm::mat3(
      dcd[0], 0.5f*dcd[1], 0.5f*dcd[2],
      0.5f*dcd[1], dcd[3], 0.5f*dcd[4],
      0.5f*dcd[2], 0.5f*dcd[4], dcd[5]);
    glm::mat3 dL_dM = 2.0f * M * dL_dSigma;
    glm::mat3 Rt = glm::transpose(R);
    glm::mat3 dL_dMt = glm::transpose(dL_dM);
    glm::vec3* dsd = dL_dscale + idx;
    dsd->x = glm::dot(Rt[0], dL_dMt[0]);
    dsd->y = glm::dot(Rt[1], dL_dMt[1]);
    dsd->z = glm::dot(Rt[2], dL_dMt[2]);
    ...
    dL_dq.z = 2*x*(dL_dMt[1][0]+dL_dMt[0][1])+2*r*(dL_dMt[2][0]-dL_dMt[0][2])+2*z*(dL_dMt[1][2]+dL_dMt[2][1])-4*y*(dL_dMt[2][2]+dL_dMt[0][0]);
    dL_dq.w = 2*r*(dL_dMt[0][1]-dL_dMt[1][0])+2*x*(dL_dMt[2][0]+dL_dMt[0][2])+2*y*(dL_dMt[1][2]+dL_dMt[2][1])-4*z*(dL_dMt[1][1]+dL_dMt[0][0]);
    float4* drd = (float4*)(dL_drot + idx);
    *drd = float4{ dL_dq.x, dL_dq.y, dL_dq.z, dL_dq.w };
  }    
  ...
  float* __restrict__ dL_dextras)
{
  ...
  // Clamp: when alpha_sum ≈ 1.0, FP rounding makes 1-alpha_sum a tiny non-zero.
  // Repeated T = T/(1-alpha) then overflows to INF after ~120 Gaussians.
  // Force T_final = 0 when alpha > 0.999 to break the overflow chain.
  const float T_final = inside ? ((accum_alphas[pix_id] > 0.9999f) ? 0.0f : (1.0f - accum_alphas[pix_id])) : 0.0f;
  float T = T_final;
