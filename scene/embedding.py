...
try:
    from jittor import Tensor  # Jittor 1.3.11 does not export Tensor; use jt.Var instead
except ImportError:
    Tensor = jt.Var  # fallback for type annotations
...
class FieldComponent(nn.Module):
    ...
    def build_nn_modules(self) -> None:
        """Function instantiates any jittor.nn members within the module.
    ...
    @abstractmethod
    def execute(self, in_tensor: Shaped[Tensor, "*bs input_dim"]) -> Shaped[Tensor, "*bs output_dim"]:
        """
...
class Embedding(FieldComponent):
    ...
    def execute(self, in_tensor: Shaped[Tensor, "*batch input_dim"]) -> Shaped[Tensor, "*batch output_dim"]:
        """Call forward
