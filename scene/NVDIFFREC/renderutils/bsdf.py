...
def _bend_normal(view_vec, smooth_nrm, geom_nrm, two_sided_shading):
    ...
    if two_sided_shading:
        mask = (_dot(geom_nrm, view_vec) > 0).float()
        smooth_nrm = mask * smooth_nrm + (1.0 - mask) * (-smooth_nrm)
        geom_nrm = mask * geom_nrm + (1.0 - mask) * (-geom_nrm)

    t = jt.clamp(_dot(view_vec, smooth_nrm) / NORMAL_THRESHOLD, min=0, max=1)
    # jt.lerp not available in Jittor: a + t * (b - a)
    return geom_nrm + t * (smooth_nrm - geom_nrm)
...
def bsdf_frostbite(nrm, wi, wo, linearRoughness):
    ...
    mask = ((wiDotN > 0.0) & (woDotN > 0.0)).float()
    return mask * res
